...
object WhatsWrong2 {
  ...
  /* We can refactor this code in one line using flatMap et map.*/
  def getCEOAndEnterprise(ceo_id: Option[String]): Future[(Option[CEO], Option[Enterprise])] = {
    ...
    CEODao.byId(ceo_id.get).flatMap(ceo => EnterpriseDao.byCEOId(ceo_id.get).map(enterprise => {(ceo, enterprise)}))
  }
}
